...
sqldf = spark.sql(
    "SELECT user.id,user.name,count(*) FROM Users WHERE (user.id is not null and user.name is not null) group by user.id,user.name order by 3 desc limit 10")
sqldf.show(150)

sqldf.toPandas().to_csv('second.csv')
# Code for bar graph
